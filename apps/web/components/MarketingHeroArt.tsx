/**
 * MarketingHeroArt — globus + plivajući chip-ovi sa univerzalnim labelama
 * (jezici, nivoi, AI). Za nelogovane posetioce na landing-u — nema personalizovanih
 * brojki (streak/percent/level) jer ih posetilac nema.
 *
 * Za logovani dashboard postoji `StudentHeroArt` sa pravim brojkama.
 */
...
function Globe() {
  return (
    <svg viewBox="0 0 200 200" className="h-full w-full">
      <defs>
        <radialGradient id="ocean-marketing" cx="38%" cy="32%" r="75%">
          <stop offset="0%" stopColor="#6fb0e3" />
          <stop offset="55%" stopColor="#3E8FD0" />
          <stop offset="100%" stopColor="#2f6fa6" />
        </radialGradient>
        <clipPath id="globe-clip-marketing">
          <circle cx="100" cy="100" r="92" />
        </clipPath>
      </defs>
      <circle cx="100" cy="100" r="92" fill="url(#ocean-marketing)" />
      <g clipPath="url(#globe-clip-marketing)">
        <g fill="#86C440">
...
function Chip({
  ...
  return (
    <div
      className={`absolute flex animate-float items-center gap-2 rounded-2xl bg-white px-3.5 py-2 text-sm font-semibold text-ink shadow-card ring-1 ring-black/5 ${className}`}
...
export function MarketingHeroArt() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-md">
      ...
      {/* Plivajući chip-ovi — univerzalni za nelogovanog posetioca */}
      <Chip className="-left-4 top-[10%]" delay="0.2s">
        <span className="text-base">🇩🇪</span>
        <span>Nemački</span>
      </Chip>

      <Chip className="-right-3 top-[26%] bg-primary text-ink" delay="1.4s">
        <span>A1 → C1</span>
        <span className="text-xs font-bold uppercase tracking-wider opacity-70">6 nivoa</span>
      </Chip>

      <Chip className="-left-2 bottom-[24%]" delay="0.8s">
        <span className="text-base">🤖</span>
        <span>AI tutor</span>
      </Chip>

      <Chip className="-right-4 bottom-[10%]" delay="2s">
        <span className="text-base">🇬🇧</span>
        <span>Engleski</span>
      </Chip>
    </div>
